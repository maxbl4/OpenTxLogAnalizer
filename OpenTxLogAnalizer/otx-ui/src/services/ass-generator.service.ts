...
import {IStatTriple, StatTriple} from "./IStats";
...
`;
  ...
  getOsd(osdLayout: string, row: any, log: ILog) {
    ...
    for (let m of matches) {
      ...
      const value = formatValue(field, row[field], padding, format, log);
...
function formatValue(field: string, value: any, padding: number, format: string, log: ILog) {
  switch (format) {
    case "barReverse":
    case "bar":
